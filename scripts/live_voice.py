...
from scripts.run_pipeline import EndToEndSTSPipeline
...
class LiveVoiceConverter:
    ...
    def __init__(
        ...
        self.pipeline = EndToEndSTSPipeline(chunk_duration_ms=1200)
        self.streaming_reasoning = StreamingReasoningOrchestrator(
            dialect_identifier=self.pipeline.dialect_identifier,
            text_normalizer=self.pipeline.text_normalizer,
            code_switch_detector=self.pipeline.code_switch_detector,
            response_generator=self.pipeline.response_generator,
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            vad_result = self.pipeline.vad.process_chunk(
                AudioChunk(
            ...
            self._process_partial_transcript_update(buffers)
            utterance = np.concatenate(buffers.frames, axis=0)
    ...
    def _finalize_turn(self, utterance: np.ndarray, transcript_text: str, sd) -> None:
        started = time.perf_counter()

        if not transcript_text:
            self.logger.warning("No partial transcript captured; using fallback turn pipeline")
            waveform = torch.from_numpy(utterance.astype(np.float32, copy=False)).contiguous()
            try:
                result = self.pipeline.run_waveform(
                    waveform=waveform,
                    sample_rate=self.sample_rate,
                    task_instruction=self.task_instruction,
                    language_hint=self.language_hint,
                    use_vad_in_asr=False,
                )
            ...
            self.logger.info("User: %s", result.transcript_text)
            self.logger.info("Assistant: %s", result.response_text)
            self.logger.info("Turn latency: %.2fs", elapsed)

            audio = result.speech_result.waveform.detach().cpu().numpy().astype(np.float32, copy=False)
            if audio.size == 0:
                self.logger.warning("TTS produced empty audio for this turn")
                self.streaming_reasoning.reset_turn()
                return

            sd.play(audio, samplerate=result.speech_result.sample_rate, blocking=True)
            self.streaming_reasoning.commit_assistant_response(result.response_text)
            self.streaming_reasoning.reset_turn()
            return
        ...
        try:
            ...
            if self.llm_streaming and hasattr(self.pipeline.response_generator, "stream_response"):
                ...
                turn_response = self.pipeline.response_generator.generate_response(prompt_input)
    ...
    def _play_streaming_llm_response(self, prompt_input, dialect_signal, sd) -> str:
        response_text, chunks = self.pipeline.response_generator.stream_response(prompt_input)
    ...
    def _speak_text_unit(self, text: str, dialect_signal, sd) -> None:
        ...
        routing_result = self.pipeline.tts_router.route_response(
            text=cleaned,
            dialect_signal=dialect_signal,
        )
        ...
        for chunk in self.pipeline.streamer.stream_routed_response(routing_result):
    ...
    def _process_partial_transcript_update(self, buffers: StreamingTurnBuffers) -> None:
        ...
        try:
            asr_result = self.pipeline.asr.transcribe_chunk(
                ASRAudioChunk(
